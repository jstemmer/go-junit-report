...
var (
	...
	skippedNodeTextLocationFlag = flag.String("skipped-node-text-location", "", `set location of <skipped> node text: text-node (child text node of <skipped> node), message-attr ("message" attribute of <skipped> node), default is text-node`)
...
func main() {
	...
	skippedNodeTextLocation := junit.TextNode
	if *skippedNodeTextLocationFlag != "" {
		var err error
		if skippedNodeTextLocation, err = junit.ParseSkippedNodeTextLocation(*skippedNodeTextLocationFlag); err != nil {
			exitf(err.Error())
		}
	}
	...
	config := gojunitreport.Config{
		...
		SkippedNodeTextLocation: skippedNodeTextLocation,
